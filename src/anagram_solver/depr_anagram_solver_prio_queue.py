...
class PartialAnagram:
    def __init__(self, char, h, sentence, remaining, last_word):
        self.char = char
        self.h = h
...
class PrioritizedItem:
    neglogprob: float=field(compare=True)
    item: PartialAnagram=field(compare=False)
...
def solve_anagram(net, chars, best_n=BEST_N, cuda=CUDA, lang="de"):
    ...
    queue = PriorityQueue()  # Stores the partial anagrams

    # Initialize the queue
    queue.put(PrioritizedItem(10000, 0, PartialAnagram(
        char="\n",
        h=net.init_hidden(1),
        sentence="",
    ...
    while not queue.empty():
        ...
        for next_char, next_prob in probs.items():
            if el.char in (" ", "\n") and next_char in (" ", "\n"):
                continue  # avoid infinite loop
            queue.put(PrioritizedItem(
                neglogprob - np.log(next_prob),
                PartialAnagram(
                    char=next_char,
                    h=h,
